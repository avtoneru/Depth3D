...
{	
	...
	float D = Depth; // по умолчанию Depth = 25, аналог MS
	float2 uv = 0;
	[loop]
	for (int j = 0; j < 4; ++j) 
	{	
...
float2 DL(float2 p, float k_RGB) //Cubic Lens Distortion 
{
...
float2 DR(float2 p, float k_RGB) //Cubic Lens Distortion 
{
